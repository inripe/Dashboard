"""
INRIPE DM TRACKING — view module (v7)
Presentation only; all arithmetic lives in dm_engine.py.
Rendered by main.py as one of two views, so this file sets no page config
and calls no st.stop() - stopping here would take the whole app down.
...
# Local fallbacks only; production reads from SharePoint.
EXCEL_CANDIDATES = [
...
# Injected from inside render(), not at module level. Module-level code runs
# once on first import, but Streamlit redraws on every rerun - styling
# applied at import time survives the first paint and nothing after it.
_CSS = """
<style>
...
def render():
    """Draw the tracking dashboard into the current container."""
    ...
    st.markdown(
        f"<div style='font-size:12px;color:#8A8A8A;margin:-6px 0 12px'>"
        f"Latest data <b style='color:#333'>"
        f"{last_dt.strftime('%d %b %Y') if pd.notna(last_dt) else 'n/a'}</b>"
        f" &nbsp;&middot;&nbsp; {stamp_label.lower()} "
        f"<b style='color:#333'>{stamp}</b>"
        f"{f' by {who}' if who else ''}</div>", unsafe_allow_html=True)
    ...
    if ALLOC.empty:
        ...
        disp = pd.DataFrame({
            ...
            "vs paced": ALLOC["vs paced"].map(lambda v: "n/a" if pd.isna(v) else f"{v:.0f}%"),
            "Budget used": ALLOC["Budget used"].map(
            ...
            "CAC": ALLOC["CAC"].map(lambda v: "n/a" if pd.isna(v) or v == 0 else f"{v:.1f}"),
            "Plan CAC": ALLOC["Plan CAC"].map(lambda v: "n/a" if pd.isna(v) else f"{v:.1f}"),
            "Cost vs plan": ALLOC["Cost index"].map(
                lambda v: "n/a" if pd.isna(v) else f"{v:.2f}x"),
            "ROAS": ALLOC["ROAS"].map(lambda v: "n/a" if pd.isna(v) or v == 0 else f"{v:.1f}x"),
            "Read": ALLOC["Read"],
        })
        table(disp,
        ...
        idle = ALLOC[ALLOC["Headroom"].fillna(0) > 0]
        if len(idle):
